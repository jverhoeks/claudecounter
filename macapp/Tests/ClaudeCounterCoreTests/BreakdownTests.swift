...
/// `breakdownRows` turns one hour's per-model USD map into the
...
final class BreakdownTests: XCTestCase {
    ...
    func test_sortsBySpendDescending() {
        let (rows, overflow) = breakdownRows(
    ...
    func test_tiesBreakByModelNameSoOrderIsStable() {
        ...
        let first = breakdownRows(from: input, limit: 5).rows.map(\.model)
        XCTAssertEqual(first, ["alpha", "beta", "gamma"])
        for _ in 0..<20 {
            XCTAssertEqual(breakdownRows(from: input, limit: 5).rows.map(\.model), first)
        }
    }

    func test_emptyHourYieldsNoRows() {
        let (rows, overflow) = breakdownRows(from: [:], limit: 5)
    ...
    func test_allZeroHourYieldsNoRows() {
        ...
        let (rows, overflow) = breakdownRows(from: ["a": 0, "b": 0], limit: 5)
        XCTAssertTrue(rows.isEmpty)
        XCTAssertEqual(overflow, 0)
    }

    func test_totalIsTheSumOfKeptRows() {
        let (rows, _) = breakdownRows(from: ["a": 1.5, "b": 2.25, "z": 0], limit: 5)
        XCTAssertEqual(rows.reduce(0) { $0 + $1.value }, 3.75, accuracy: 1e-9)
    }
}
