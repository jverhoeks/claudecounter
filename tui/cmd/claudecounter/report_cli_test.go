...
func TestWriteReportCSV(t *testing.T) {
	...
	if lines[0] != "repo,bucket,usd,commits_mine,commits_all,added,deleted,files,usd_per_commit,usd_per_line" {
	...
	// W24 row: zero ratios render as empty cells (trailing commas)
	if !strings.HasSuffix(lines[2], ",0,1,0,0,0,,") {
